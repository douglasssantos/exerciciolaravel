    ...
    <div class="mx-auto max-w-2xl text-center">
      <div class="absolute right-0 top-0">
        <div  class="p-3 flex gap-3 items-center flex-row">
          <template v-if="props.isLogged">
            <button @click="fnCreate" class="bg-blue-400 hover:bg-blue-300 text-white font-bold py-2 px-4 border-b-4 border-blue-600 hover:border-blue-400 rounded flex flex-row items-center gap-2">
              <UserPlusIcon class="h-6 w-6 text-white-500" /> Adicionar Contato
            </button>

            <a :href="route('contact.trashed')" class="bg-gray-100 hover:bg-gray-200 text-black font-bold py-2 px-4 border-b-4 border-gray-200 hover:border-gray-300 rounded  flex flex-row items-center gap-2">
              <TrashIcon class="h-6 w-6 text-white-500" /> Contatos Apagados
            </a>
      ...
      <ul role="list" class="divide-y divide-gray-100">
            ...
            <template v-if="props.isLogged">
              <div class="hidden sm:flex sm:flex-col sm:items-end">
                <div class="mt-1 flex items-center gap-x-1.5">
                  <button @click="fnShowDetail(person)" class="bg-blue-400 hover:bg-blue-300 text-white font-bold py-2 px-4 border-b-4 border-blue-600 hover:border-blue-400 rounded">
                    <EyeIcon class="h-5 w-5 text-white-500" />
                  </button>
                  <button @click="fnUpdate(person)" class="bg-orange-400 hover:bg-orange-300 text-white font-bold py-2 px-4 border-b-4 border-orange-600 hover:border-orange-400 rounded">
                    <PencilSquareIcon class="h-5 w-5 text-white-500" />
                  </button>
                  <button @click="fnDelete(person.id)" class="bg-red-500 hover:bg-red-400 text-white font-bold py-2 px-4 border-b-4 border-red-700 hover:border-red-500 rounded">
        ...
        <modal-contact-detail
            :show="state.modalDetail.show"
            :data="state.modalDetail.data"
            @close="fnCloseDetail"
            @event="fnEventDetail"/>
      </ul>
  </div>
...
<script setup>
import {PencilSquareIcon, TrashIcon, UserPlusIcon, ArrowRightOnRectangleIcon, ArrowLeftOnRectangleIcon, MagnifyingGlassIcon, EyeIcon} from "@heroicons/vue/20/solid";
...
function fnShowDetail(data) {
  state.modalDetail.data = data;
  state.modalDetail.show = true;
}
function fnEventDetail(data) {
  state.modalDetail.show = data.show;
  if(data.event === "edit"){
    return fnUpdate(data.data)
  }else if(data.event === "delete"){
    return fnDelete(data.id);
  }
}
function fnCloseDetail(data) {
  state.modalDetail.show = data;
}
...
function fnDelete(id) {
  axios.delete(route("contact.destroy", id)).then(
      response => {
...
function fnSearch() {
  axios.post(route('contact.search'), {
